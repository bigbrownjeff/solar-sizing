...
All four flags default to off/zero, so every pre-existing run produces byte-identical output.
...
| File | What it is |
|---|---|
| `index.html` | **The map calculator** (headline). Your roof on Esri satellite imagery with the two estimated panel planes drawn at the building's real orientation; drag to move, drag a corner to resize, set tilt/azimuth, and every number recomputes live from a client-side yield model over **live NASA POWER 0.5° point data** (monthly GHI + temperature; an embedded coarse grid offline), validated to ~3% mean error vs PVGIS across five sites. Only a coordinate is sent to public APIs, never your address; self-contained, no build step. A built-in panel lists finer free and paid irradiance sources. |
